...
# Install Git
RUN apt-get update && apt-get install -y git clang build-essential cmake libboost-all-dev
...
RUN mkdir Programs/bin/
RUN cd Programs/TeloPort-master/ && make
RUN mv Programs/TeloPort-master/build/apps/* Programs/bin/
