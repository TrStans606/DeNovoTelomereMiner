	...
	for x in range(lineCnt):
		if lines[x][0] == '@' and x % 4 == 0:
			...
			try:
				...
				try:
					lines[x + 1].split("CCCTAACCCTAA")[1]
				except:
					pass
				else:
					telos[directory + str(x)] = (lines[x + 1], "CCCTAA")
			else:
...
def gffBuilder():
	...
	for x in range(linecnt):
		...
		command = lines[x].split('\t')[1] + '\t' + 'teloPortWrapper' + '\t' + 'telomere' + '\t' + lines[x].split('\t')[8] + '\t' + lines[x].split('\t')[9] + '\t' + lines[x].split('\t')[10] + '\t' + strand + '\t' + '.' + '\t' + 'color=' + color + ';ID=Telomere' + str(x) + ';Name=' + lines[x].split('\t')[0] + ';Length=' + str(length) + ';Seq=' + telos[lines[x].split('\t')[0]][0] + ';Distance from Telomere=' + lines[x].split('\t')[14] + '\n' 
		write.write(command)
